...
1. Ensure the API is running: `docker compose up` or `./run.sh`
2. Open Streamlit UI at http://localhost:8501
3. Have the list of applicants ready (APP001–APP020)
...
- No user authentication in current build
...
- **Debugable** — Each agent's output can be inspected independently
- **Maintainable** — Update one prompt without affecting others
- **Safe** — Short-circuit on intake/doc failure saves 4 LLM calls
- **Auditable** — Full per-agent trace for every screening

### Safety by Design

The system has three built-in safety mechanisms:

1. **Post-LLM rule validation** — Python code overrides the LLM on hard facts
2. **Human review disclaimer** — Every output requires officer verification
3. **No protected attributes** — Schema excludes gender, religion, race, etc.
